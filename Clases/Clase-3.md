...
Supongamos que hacemos una encuesta sobre un tema, donde la respuesta de la gente es si o no. Si muestramos 50 personas, el espacio muestral es 2 a la 50, esa es la cantidad de posibles respuestas. Ahora bien, quizas no me interesa quien respondio que, sino el porcentaje de personas que respondio algo.
Por ejemplo podemos tomar X = 'numero de personas que respondio si', donde X ahora forma un espacio muestral de 50 numeros.
...
Es como si se definiera un nuevo espacio muestral donde se mueve la variable aleatoria. 

Supongamos que tenemos un espacio muestral $S = \{S_1, ...., S_n\}$, mas una funcion de probabilidad $P$ y una variable aleatoria, $X = \{X_1, ..., X_m\}$. Definimos la funcion de probabilidad como $P_x$ ,donde X es la variable, de la siguiente manera: 
- Observamos el valor $X = x_{i}$ (En mayuscula hablo de la vairble, en minuscula del valor), si y solo si el resultado del experimento es $S_{j} \in S$ tal que $X(S_{j}) = x_{i}$. Luego $P_{X}(X = x_{i}) = P(\{ S_{j} \in S : X(S_{j}) = x_{i}\})$. A esto se lo llama **probabilidad inducida**.
El valor de la variable aleatoria, queda determinada por lo eventos del espacio muestral que cumpla con su valor. 
- $P_{X}(X = X_{i})$ se suele contar como $P(X = x_{i})$ o $P(X = x_{i})$. Lo que expresa es la funcion de probabilidad asociada a una variable aleatoria $X$,asociada a un valor determinados $x_{i}$. 
...
- $P_{x}(X = 27) = \frac{\binom{50}{27}}{2^{50}}$, tomar de 50, las 27 personas que pueden poner que si, sobre los escenarios totales. 
En general:
- $P_{x}(X = i) = \frac{\binom{50}{i}}{2^{50}}$
...
Para $P_X{X}(X = 0) = 1/8$, luego $P(X = 1) = 3/8$, luego $P(X = 2) = 3/8$ y $P(X = 3) = 1/8$, luego acumuladno tenemos la $fda$:
Luego la $fda$ sera: 
...
Y esta es la llamada **distribucion geometrica**. Cualquier problema que se pueda mapear con cuantas tiradas hacen falta para encontrar un elemento bsucado al final, se puede resover con esto. Template: numero de ensayos hasta conseguir el primer exito.
...
la $fdp$ de una variable aleatoria discreta $X$ es:
- $f_{X}(x) = P(X = x)$
...
En varibales continuas, si quiero preguntar por la porbabilidad de un valor exacto es 0. Es decir la porbabilidad de un valor puntual sera 0, mas alla de que el intervalo tenga un valor cerrado o abierto. De esta forma, para v.a continuas, la porbabilidad de $P(a < x < b) = P(a <= x <= b) = P(a <= x < b) = P(a < x <= b)$.
...
Lo que dicen estan ultimas dos porpiedades es que la porbabilida de que sea cualquier valor es igual a 1. Esto esta en correlacion a cuandod deciamos que $P(S) = 1$, siendo $S$ el espacio muestral.

### VALOR ESPERADO
Una forma de resumir los valores de una variable aleatoria, es condensarlo mediante un valor. Una forma de conseguir esto es por medio de la esperanza o valor esperado. El valor esperado de una variable aleatoria $X$ llamado $E(X)$ es:
...
- Donde $g(x) = x - E(X)$ Donde este valor que utilizo en la formula es por ejemplo el promedio, algo que calcule antes. Esto representa por ejemplos distancias al pormedio o valores medios. La idea es realizar un promedio con esto, pero que no se compensen entre las positivas y negativas. Quiero ver cuanto se aparta cada valor de su propio medio. Para evitar esto de los positivos negativos lo pienso como $g(x) = (x -  E(X))^{2}$. El valor esperado de esta expresesion $E[(x - E(X))²] = varianza$. Lo que nos interesa saber es que distancia promedio hay entre los valores y su medio. Se suele nomeclar como $VAR(X)$, que temina siendo el valor esperado de la funcion que definimos. 

si analizamos la esperanza, $f_{x}(X)$ no posee unidad, por lo tatnto lo que determina la unidad sera la expresion de $g(x)$. Lo mimso para la varianza, sera la unidad de la funcion $g(x)$ al cuadrado. A partir de esto diremos que $\sqrt VAR(X) = la desviacion estandar$
...
## NOTAS PRACTICAS
Desde el punto de vista grafico, la funcion de distribucion acumulada se puede pensar como el area bajo la cursa, es lo que se acumula entre dos puntos dados. Por eso siempre es util entender esto para saber de donde viene la idea de integrar sobre la funcion puntual dada. 

El grafico de la misma tmb puede ser util para pensar la porbabilida que hay entre un intervalos. A partir de entender que intervalos debe sacar o calccular por medio de la funcion, es una idea grafica de lo que deberia valer la probabilidad. 
