...
Son los fundamentos sobre los que se construye toda la estadistica. Dando mecanismo para poder modelar poblaiciones o experimientos ubicados dentro de lo que se consideran como fenomenos aleatorios. 
...
El espacio muestral puede ser contable, si tiene una correspondencia 1 a 1 con los enteros, o en caso contrario puede ser considerado como incontable. Siempre que es finito va a ser contable. 
...
Los sets son conjuntos, por lo tanto aplican las operaciones de union, interseccion y complemento, entre otras. Sobre esto tambien valen propiedades de distribucion, asociatividad, distribucion y leyes de de morgan.
...
La frecuencia con la que ocurre un resultado producto de una experiementacion se puede considerar como una probabilidad. No se suele definir probabilidad en terminos de frecuencia, sino desde un punto de vista matematico. Se busca hacer que la probabilidad se defina por medio de una funcion que cumple ciertos axiomas. 
...
De ser finito el **Sigma algebra** sera el cojunto de partes. Cuando es finito el sigam algebra es mas sencillo de calcular.
...
**TEOREMA:** sea **S** un espacio muestral finito. Suponiendo que tengo un **Sigma algebra** de subconjuntos de **S**. Sea $p_1, ...., p_n$ numeros no negativos que suman 1. Para todo **A** perteneciente a el **Sigma Algebra**, define $P(A)$ como: 
...
## CONTEO
Se utilizan distintos metodos de conteo para construir asignacion de probabilidades en espacios de muestreos finitos. Para resolver esto suele ser util dividirlos en tareas mas pequeñas y aplicar reglas conocidas para combinarlos. 
...
Si dice **without replacement**, implica que se puede repetir. En caso contrario indica que no se puede repetir el elemento.
...
- P(A|B): La probabilidad posterior. la probabilidad de que ocurrio A, dado el evento B. conocida como **probabilidad a posteriori**.
- P(B|A): La probabilidad previa, antes de la evidencia. es la probabilidad de que ocurra B dada que la hipotesis A es verdadera. probabilidad de B dado A. Esta se la conoce como **verosimilitud** o **likehood**
- P(A): probabilidad de la evidencia, dado que la creencia es verdadera. Esta se la conoce como la prioridad **a priori**.
- P(B): probabilidad marginal, probabilidad de la evidencia bajo cualquier circunstancia. ver cual es la probabilidad de que ocurra mas alla de cualquier condicion. se la conoce como **evidencia**.
...
$P(B|A^c)P(A^c)$ la probabilidad de ver B en el mundo donde no ocurre A.
...
- Una coleccion de eventos $A_1,A_2,..$ son independientes entre ellos si para toda subcoleccion $A_{i}{1},...,A_{i}{k}$ tenemos que:
...
En conclusiones la **Distribucion** describe como se reparte la probabilidad sobre los posibles valores de una variable aleatoria. La nocion de distribucion va mas alla de la fruncion que usamos, las funciones (cdf, pmf, pdf) son formas concretas de describirla.
...
Notar que para hablar de estas funciones le ponemos una letra f en minuscula.
...
Para la definicion de esta funcion en el caso de las variables independientes discretas, podemos sumar los valores de la pmf para obtener la cdf.

para el caso de las variables aletorias continuas usamos otro nombre.

En el caso de varibales aletorias discrtes no se puede preguntar por igualda frente a la distrbucion, debido a que esta se piden sobre un intervlo infinito.
...
Esto ocurre porque para variables continuas ocurre que la porbabilidad de que sea un valor puntutal es igual a 0, es decir para X siendo una variabel continua, tenemos que $P(X= x) = 0$
...
Como la funcion acumulativa es la integral sobre el intervalo de la funcion de densidad, y la densidad es la derivada de la funcion de la acumulativa, con solo conocer una pudo aplicar la inversa y obtener la misma informacion.
...
Una transformacion no cambia las porbabilidades, solo la forma en la que se observa. Reagrupa los valores de **X** en terminos de **Y**. Esto es importane, porque por medio de una funcion de probabilidad ya calcualdao puedo observar un nuevo fenomeno.
...
En resumen en la esperanza sera un valor que nos indica el valor promedio que se espera obtener de una variable aleatoria a largo plazo. Es decir, si repito el experimento muchas veces, la esperanza nos dice cual sera el resultado promedio. Nos da una idea de un comportamiento general a largo plazo. Es decir, para los distintos valores que puedoe obtener para esta variables aletaoria, cual seria el promedio. 
...
La Ley de los Grandes Números garantiza que, a medida que el número de observaciones reales (\(n\)) crece y se acerca al infinito, la media muestral se aproximará cada vez más a la esperanza matemática [STEM]

Eso nos especifica hacia donde viaja la media meustral, donde converge hacia la esperanza.

**CONVERGENCIA A DISTRBUCION:**
![Texto alternativo](Convergencia-A-Distribucion.png)

Este concepto describe qué pasa cuando no solo la media se estabiliza, sino que toda la forma de la distribución de una secuencia de variables aleatorias se transforma y se asienta en una distribución límite conocida a medida que \(n\) tiende a infinito

**TEOREMA CETRAL DEL LIMITE:**
![Texto alternativo](Teorema-Del-Limite-Central.png)

Este termina siendo un teorema de convergencia de la distribucion.
...
En este caso nos dice como viaja la media muestra, como fluctua  alo largo del camino. 
